...
class LoginViewModel @Inject constructor(
    ...
    fun onLoginClicked(user: String, pass: String) {
        viewModelScope.launch {
            ...
            // Llamamos al repositorio (que busca en la BD)
            val isSuccess = repository.login(user, pass) // Aquí busca por email o username

            if (isSuccess) {
                _loginChannel.send(LoginEvent.Success)
            } else {
                _loginChannel.send(LoginEvent.Error("Credenciales incorrectas"))
            }
        }
    }
}